...
async function createConfigFiles(): Promise<boolean | never>{
    if(!mapsFile) throw Error('Environment variables missing');
    const filePath = getConfigFilePath(mapsFile);
    const exists = await fileExists(filePath);
    if(!exists) await fs.writeFile(filePath, '{}', 'utf8');
    return true;
}
 ...
 */
async function writeMaps(event: IpcMainInvokeEvent, data: string): Promise<boolean | never>{
    const filePath = getConfigFilePath(mapsFile!);
 ...
 * File must be utf8 encoded
 */
async function readMaps(): Promise<string | never>{
    const filePath = getConfigFilePath(mapsFile!); 
